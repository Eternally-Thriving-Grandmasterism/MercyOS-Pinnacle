...
fn setup(
    ...
    let spine_lower = commands.spawn((
        PbrBundle {
    ...
    let spine_upper = commands.spawn((
        PbrBundle {
            mesh: bone_mesh.clone(),
            material: skin_material.clone(),
            transform: Transform::from_xyz(0.0, 0.4, 0.0),
    ...
    commands.entity(spine_root).push_children(&[spine_lower]);
    commands.entity(spine_lower).push_children(&[spine_mid]);
    commands.entity(spine_mid).push_children(&[spine_upper]);
    commands.entity(player_body).push_children(&[spine_root]);

    // Arms mercy (as before)
...
fn multi_chain_ik_system(
    ...
    mut chain_query: Query<&mut Transform, Or<(With<SpineLower>, With<SpineMid>, With<SpineUpper>, With<LeftUpperLeg>, With<LeftLowerLeg>, With<RightUpperLeg>, With<RightLowerLeg>)>>,
    ...
    // Spine to head mercy (as before)

    // Left leg IK mercy
    if let (Ok(mut left_upper_leg), Ok(mut left_lower_leg), Ok(left_foot)) = (
        chain_query.get_component_mut::<Transform>(/* left_upper_leg entity */),
        chain_query.get_component_mut::<Transform>(/* left_lower_leg entity */),
        ...
        let lengths = [0.5, 0.5];  // Thigh + shin mercy

        fabrik_ik_multi_chain(&mut positions, &lengths, left_foot.translation, 0.01, 10);
