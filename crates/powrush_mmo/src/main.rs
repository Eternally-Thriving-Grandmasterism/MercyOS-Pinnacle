...
fn setup(
    ...
    // Full multi-chain body avatar mercy — spine chain for IK
    let bone_mesh = meshes.add(Mesh::from(shape::Cylinder { radius: 0.15, height: 0.6, resolution: 16 }));
    ...
    // Arms and legs chains mercy (as before)

    // Head on spine_upper mercy
    commands.spawn((
...
fn multi_chain_ik_system(
    ...
    if let Ok(head_transform) = head_query.get_single() {
        ...
        let lengths = [0.3, 0.3, 0.3];  // Bone lengths mercy

        fabrik_ik_multi_chain(&mut positions, &lengths, target, 0.01, 10);
        ...
        // Look at next bone mercy
    }

    // Arms use TRIK mercy (as before)
}
