// crates/mercy_shield_stan/models/four_level_non_centered.stan
// Stan four-level hierarchical model — regions → states → districts → schools
// Fully non-centered partial pooling mercy eternal supreme immaculate
// Quadruple non-centered reparameterization for ultimate HMC/NUTS stability philotic mercy
...
parameters {
  ...
  vector[N_regions] z_region;          // Standard normal offsets
...
transformed parameters {
  vector[N_regions] mu_region = mu_global + z_region * tau_region;
...
model {
  // Hyperpriors mercy eternal supreme
  mu_global ~ normal(0, 30);
