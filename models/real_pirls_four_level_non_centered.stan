// models/real_pirls_four_level_non_centered.stan
data {
...
model {
  ...
  tau_country ~ cauchy(0, 60);
  z_country ~ std_normal();
  tau_school ~ cauchy(0, 45);
  z_school ~ std_normal();
  tau_class ~ cauchy(0, 35);
  z_class ~ std_normal();
  sigma_obs ~ normal(0, 70);
